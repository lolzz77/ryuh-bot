...
TOKEN = 'MTA2MjM4MTY0MTQ0NTY4NzMyNg.G84kjk.G0sdSEyAiV1E0a_a6Zh-DjTDl5za6RmFmAwtNk'
...
async def on_message(message):
    if message.content.lower() == 'ryuh bot':
        ...
        json_data = {
            "schedule" : "weekend",
            "date_generated" : str(scheduler.now),
            "message_id" : msg_id
        }

        file_path = scheduler.SCHEDULE_PATH + str(message.channel.id)
        ...
        if(False == isExist):
            ...
            json_obj = json.dumps(json_array)
            f.write(json_obj)
        else:
            f = open(file_path, "r")
            raw_data = f.read()
            ...
            for item in parsed_data:
                if item["schedule"] == "weekend":
                    item["date_generated"] = str(scheduler.now)
                    item["message_id"] = msg_id
                    f = open(file_path, "w")
                    json_obj = json.dumps(parsed_data)
                    f.write(json_obj)
        f.close()
    ...
    if message.content.lower() == 'ryuh weekday' or message.content.lower() == 'ryuh weekend':
        # Send schedule message to channel
        msg_sent = await message.channel.send("This command is deprecated. Please use 'ryuh boss' instead.")
    ...
    if message.content.lower() == 'ryuh check':
        ...
        for item in parsed_data:
            ...
            start_of_week = datetime.datetime.timestamp(scheduler.monday_midnight)*1000
            end_of_week = datetime.datetime.timestamp(scheduler.sunday_before_midnight)*1000
            # logic is like this
            # if passsed sunday midnight, then only check weekday
            # if not yet pass sunday mightnight, then check both weekda & weekend
            if date_generated <= end_of_week:
                x = 1
            # elif start_of_week >= date_generated:
        f.close()
